...
void loop() {
  ...
  if (DW1000NgRTLS_MT::receiveFrame()) {
    ...
    if (recv_data[0] == BLINK) {
      DW1000NgRTLS_MT::transmitRangingInitiation(&recv_data[2], tag_shortAddress);
      DW1000NgRTLS_MT::waitForTransmission();
      result = DW1000NgRTLS_MT::anchorRangeAccept(NextActivity::RANGING_CONFIRM, next_anchor);
      if (result.success) {
        size_t len = recv_len;
        byte data[len] = {};
